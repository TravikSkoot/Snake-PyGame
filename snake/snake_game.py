...
# Dynamischer Pfad für icon.ico, falls es als zusätzliche Datei eingebunden ist
...
# Icon laden mit dynamischem Pfad
...
# Highscore-Pfad basierend auf Betriebssystem festlegen
def get_high_score_path():
    if platform.system() == "Windows":
        # Windows: AppData-Verzeichnis verwenden
        appdata_path = os.getenv('APPDATA')  # 'C:\\Users\\<Benutzername>\\AppData\\Roaming'
        game_folder = os.path.join(appdata_path, 'SnakePyGame')
        os.makedirs(game_folder, exist_ok=True)
        return os.path.join(game_folder, 'highscore.txt')
    else:
        # Andere Betriebssysteme: Im aktuellen Verzeichnis speichern
        return os.path.join(os.getcwd(), 'highscore.txt')

# Highscore laden
def load_high_score():
    try:
        with open(get_high_score_path(), "r") as file:
...
def save_high_score(score):
    with open(get_high_score_path(), "w") as file:
